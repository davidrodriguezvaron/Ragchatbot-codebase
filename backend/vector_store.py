...
class VectorStore:
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict]:
        """
        Get course outline with lesson list for a given course name.

        Args:
            course_name: Course name (partial matches supported via semantic search)

        Returns:
            Dict with title, course_link, lessons list — or None if not found
        """
        import json

        # Resolve partial name to exact title
        course_title = self._resolve_course_name(course_name)
        if not course_title:
            return None
